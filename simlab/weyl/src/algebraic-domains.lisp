...
(make::adjust-version-numbers Weyl "1.18")
...
;; more complex set structures, the binary= operation looksup the
...
(defclass has-comparison ()
     ((greater-function :initform #'binary>
...
(defsubst abelian-semigroup? (domain) (typep domain 'abelian-group))
...
(defclass factor-domain (domain)
     ((factor-numer :initarg :numerator
		    :accessor factor-numer-of)
      (factor-denom :initarg :denominator
...
(defclass tpower-series (power-series)
  ((valence :initform 0 :initarg :valence :reader valence)
   (branch-order :initform 1 :initarg :branch-order :reader branch-order)
   (order :initform *positive-infinity* :initarg :order :reader order)
   (coeffs :initarg :coeffs :reader coeffs)))
...
(defclass ideal (module)
     ((ring :initarg :ring :reader ring-of)
