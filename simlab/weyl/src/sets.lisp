...
(defmethod ref ((tuple tuple) &rest args)
...
(defmethod map-with-domain (type domain function (seq tuple) &rest sequences)
  (make-instance type
		 :domain domain
		 :values (loop with v = (value-of seq)
			       for i below (array-dimension v 0)
			       collect (%apply function (aref v i)
					       (loop for seq in sequences
						     collect (ref seq i))))))
...
(defmethod set-elements ((set set-with-element-list))
...
(defmethod size ((set set-with-element-list))
  (cl:length (set-elements set)))
