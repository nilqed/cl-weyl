...
(defvar *filename-extensions*
  (car `(#+(and Symbolics Lispm)              ("lisp" . "bin")
	 ...
	 #+(and Lucid PA)                    ("lisp" . "hbin")   
         #+excl                               ("lisp"   . "fasl")
;;;  Changed the above from "cl" to "lisp" so we could use the same
...
(defun operate-on-system (name operation &key force
  ...
  (unwind-protect
	...
	(when (null force);; defaults
	  (case operation 
	    ((load :load)
	     ;; Comment this out so that if you don't reload a system twice
	     ;; accidently.  --RZ  4/27/95
	     	#+ignore(setq force :all))
